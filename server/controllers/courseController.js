...
const singleCourseDetails = async (req, res, next) => {
  try {
    let Singledetails = await courseModel
...
const verifyPayment = async (req, res, next) => {
  ...
  try {
    ...
    console.log(req.user._id, " userrrrrrrrrrrrr");
    ...
    if (razorpay_signature === expectedSign) {
     ...
     const gr= await groupModel.findOneAndUpdate(
        { _id: req.body.course.groupId },
        {
          $push: {
    ...
    //  
      ...
      sendConfirmation.sendPaymentConfirmationEmail(
        req.session.email,
        req.body.course.coursename
      );
